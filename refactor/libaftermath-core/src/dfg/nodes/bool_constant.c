...
{
	struct am_dfg_bool_constant_node* in =
		(struct am_dfg_bool_constant_node*)n;
	...
	struct am_object_notation_node_group* g)
{
	...
	if(am_object_notation_eval_retrieve_uint64(&g->node, "value", &val64))
		return 1;
	...
	struct am_object_notation_node_group* g)
{
	...
	return am_object_notation_node_group_build_add_members(
		...
		AM_OBJECT_NOTATION_BUILD_UINT64, (uint64_t)in->value,
