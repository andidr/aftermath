...
int am_dfg_histogram_builder_double_node_init(struct am_dfg_node* n)
{
	struct am_dfg_histogram_builder_double_node* hb =
		(typeof(hb))n;
...
int am_dfg_histogram_builder_double_node_process(
	struct am_dfg_node* n)
{
	struct am_dfg_histogram_builder_double_node* hb =
		(typeof(hb))n;
	struct am_dfg_port* pin = &n->ports[0];
	struct am_dfg_port* pout = &n->ports[1];
	struct am_histogram1d_double* h;
	double min = DBL_MAX;
	double max = DBL_MIN;
	double* samples;
	...
	for(size_t i = 0; i < pin->buffer->num_samples; i++)
		if(am_histogram1d_double_add_sample(
			   h, samples[i]))
		{
			goto out_err_destroy;
		}
...
out_err_destroy:
	am_histogram1d_double_destroy(h);
out_err_free:
	free(h);
out_err:
	return 1;
...
int am_dfg_histogram_builder_double_node_set_property(
	struct am_dfg_node* n,
	const struct am_dfg_property* property,
	const void* value)
{
...
int am_dfg_histogram_builder_double_node_get_property(
	const struct am_dfg_node* n,
	const struct am_dfg_property* property,
	void** value)
{
...
int am_dfg_histogram_builder_double_node_from_object_notation(
	struct am_dfg_node* n, struct am_object_notation_node_group* g)
{
	struct am_dfg_histogram_builder_double_node* hb =
		(typeof(hb))n;
...
int am_dfg_histogram_builder_double_node_to_object_notation(
	struct am_dfg_node* n, struct am_object_notation_node_group* g)
{
	struct am_dfg_histogram_builder_double_node* hb =
		(typeof(hb))n;
	...
	return am_object_notation_node_group_build_add_members(
		...
		(uint64_t)hb->auto_min_max,
		AM_OBJECT_NOTATION_BUILD_MEMBER, "min",
		AM_OBJECT_NOTATION_BUILD_DOUBLE, hb->min,
		AM_OBJECT_NOTATION_BUILD_MEMBER, "max",
		AM_OBJECT_NOTATION_BUILD_DOUBLE, hb->max);
}
