...
static inline int {{t.name}}_process(struct am_io_context* ctx, {{t.c_type}}* f)
{
	...
	if(!(a = am_event_collection_find_or_add_event_array(&ctx->trace->event_array_registry, e, {{pargs.ecoll_array_type_id}}))) {
