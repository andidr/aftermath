...
/* Finds out which chunk_set part dominates the interval defined by
 * [start; end]. If the time of two or more chunk_set parts is identical, the
 * type with the lowest id takes precedence. The filter f is evaluated
 * on each of the chunk_set parts that overlap into the interval. Chunk
 * parts that are not filtered out are taken into account for the
 * determination of the dominant chunk_set part. If there is a dominant chunk_set part,
 * the function returns the id of the the chunk_set parts. If no chunk_set part overlaps
...
{
	int idx_start = event_set_get_first_chunk_set_part_in_interval(es, start, end);
	int64_t imajor = -1;
	int64_t itmp;
	uint64_t istart;
	uint64_t iend;
 ...
 /*
 * Returns the index of the first chunk_set part in es->omp_for_chunk_set_part that
 * lies entirely within [interval_start; interval_end]. If no chunk_set part
 * event exists that covers the specified interval, the function
 * returns -1.
 */
int event_set_get_first_chunk_set_part_in_interval(struct event_set* es, int64_t interval_start, int64_t interval_end)
{
	...
	while(end_idx - start_idx >= 0) {
		...
		if(es->omp_for_chunk_set_parts[center_idx].start > interval_end)
			end_idx = center_idx - 1;
		else if(es->omp_for_chunk_set_parts[center_idx].end < interval_start)
			start_idx = center_idx + 1;
		else
			break;
	}
	...
	while(center_idx > 0 &&
	      es->omp_for_chunk_set_parts[center_idx - 1].start < interval_end &&
	      es->omp_for_chunk_set_parts[center_idx - 1].end > interval_start)
		center_idx--;
