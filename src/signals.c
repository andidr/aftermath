	...
	if(show_derived_counter_dialog(&g_mes, &opt)) {
		switch(opt.type) {
			...
			case DERIVED_COUNTER_NUMA_CONTENTION:
				err = derive_numa_contention_counter(&g_mes, &cd, opt.name, opt.numa_node, opt.data_direction, opt.contention_type, opt.contention_model, opt.num_samples, opt.cpu);
				break;
