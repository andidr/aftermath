...
#define DEFAULT_TYPED_ARRAY_PREALLOC 32
...
#define DECL_TYPED_ARRAY_EXTRA_FIELDS(prefix, T, extra_fields)		\
	struct prefix {						\
		size_t num_elements;					\
		size_t num_free;					\
		size_t num_prealloc;					\
	...
	{								\
		return add_buffer_grow((void**)&a->elements,		\
				       e,				\
				       sizeof(T),			\
				       &a->num_elements,		\
				       &a->num_free,			\
	...
	{								\
		if(check_buffer_grow((void**)&a->elements,		\
				     sizeof(T),			\
				     a->num_elements,			\
				     &a->num_free,			\
				     a->num_prealloc))			\
	...
	static inline int prefix##_prealloc_n(struct prefix* a,	\
					      size_t n)		\
	{								\
		return check_buffer_grow_n((void**)&a->elements,	\
					   sizeof(T),			\
					   a->num_elements,		\
					   &a->num_free,		\
					   n,				\
	...
	{								\
		a->num_elements = 0;					\
		a->num_free = 0;					\
		a->num_prealloc = DEFAULT_TYPED_ARRAY_PREALLOC;	\
		a->elements = NULL;					\
	}								\
									\
...
#define DECL_TYPED_ARRAY(prefix, T) \
	DECL_TYPED_ARRAY_EXTRA_FIELDS(prefix, T, )
...
#define DECL_TYPED_ARRAY_BSEARCH_SUFFIX(prefix, suffix, T, NEEDLE_T, ACC_EXPR)	\
	/* Finds the address of the element whose value is needle or NULL if */ \
	/* no such element exists. */						\
	static inline T* prefix##_bsearch##suffix(struct prefix* a,		\
						  const NEEDLE_T needle)	\
	{									\
		size_t l = 0;							\
		size_t r = a->num_elements-1;					\
		size_t m;							\
		NEEDLE_T curr;							\
										\
		if(a->num_elements == 0)					\
...
#define DECL_TYPED_ARRAY_BSEARCH(prefix, T, NEEDLE_T, ACC_EXPR) \
	DECL_TYPED_ARRAY_BSEARCH_SUFFIX(prefix, , T, NEEDLE_T, ACC_EXPR)
 ...
 * parameter. */
#define DECL_TYPED_ARRAY_BSEARCH_FIRST_SUFFIX(prefix, suffix, T, NEEDLE_T,	\
					      ACC_EXPR, SMALLER_EXPR, GREATER_EXPR) \
	/* Finds the address of the first element whose value is needle or NULL */ \
	/* if no such element exists. */					\
	static inline T* prefix##_bsearch_first##suffix(struct prefix* a,	\
							      const NEEDLE_T needle) \
	{									\
		size_t l = 0;							\
		size_t r = a->num_elements-1;					\
...
#define DECL_TYPED_ARRAY_BSEARCH_FIRST(prefix, T, NEEDLE_T, ACC_EXPR,		\
				       SMALLER_EXPR, GREATER_EXPR)		\
	DECL_TYPED_ARRAY_BSEARCH_FIRST_SUFFIX(prefix, , T, NEEDLE_T, ACC_EXPR,	\
					      SMALLER_EXPR, GREATER_EXPR)
 ...
 * search value passed to the search function. ACC_EXPR must be a macro that
 * takes one argument of type T and that returns the value that is compared to
 * the search value. */
#define DECL_TYPED_ARRAY_REMOVE_SUFFIX(prefix, suffix, T, NEEDLE_T,ACC_EXPR)	\
...
#define DECL_TYPED_ARRAY_REMOVE(prefix, T, NEEDLE_T, ACC_EXPR)			\
	DECL_TYPED_ARRAY_REMOVE_SUFFIX(prefix, , T, NEEDLE_T, ACC_EXPR)
...
#define DECL_TYPED_ARRAY_INSERTPOS_SUFFIX(prefix, suffix, T, NEEDLE_T, ACC_EXPR)\
...
#define DECL_TYPED_ARRAY_INSERTPOS(prefix, T, NEEDLE_T, ACC_EXPR) \
	DECL_TYPED_ARRAY_INSERTPOS_SUFFIX(prefix, , T, NEEDLE_T, ACC_EXPR)
 ...
 * same values as those used in DECL_TYPED_ARRAY. suffix is simply appended to
 * the name of the declared function, without a leading underscore. NEEDLE_T is
 * the type of the search value passed to the search function. ACC_EXPR and
 * NEEDLE_T must be identical the parameters passed to the macro used for the
 * declaration of the function finding the position for insertion.
 */
#define DECL_TYPED_ARRAY_RESERVE_SORTED_SUFFIX(prefix, suffix, T, NEEDLE_T, ACC_EXPR) \
...
#define DECL_TYPED_ARRAY_RESERVE_SORTED(prefix, T, NEEDLE_T, ACC_EXPR) \
	DECL_TYPED_ARRAY_RESERVE_SORTED_SUFFIX(prefix, , T, NEEDLE_T, ACC_EXPR)
