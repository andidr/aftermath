...
int am_dfg_amgui_toolbar_togglebutton_process(struct am_dfg_node* n)
{
	...
	if(am_dfg_port_activated(pout)) {
		checked = l->togglebutton->isChecked();

		if(am_dfg_buffer_write(pout->buffer, 1, &checked))
			return 1;
	}
