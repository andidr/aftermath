...
static const size_t DOUBLE_MAX_DECIMAL_DIGITS = 3 + DBL_MANT_DIG - DBL_MIN_EXP;

int am_dfg_double_to_string_node_init(struct am_dfg_node* n)
{
	struct am_dfg_double_to_string_node* tts = (typeof(tts))n;
...
int am_dfg_double_to_string_node_process(struct am_dfg_node* n)
{
	...
	if(tts->pretty_print) {
		 ...
		 * character. */
		alloc_bytes = DOUBLE_MAX_DECIMAL_DIGITS + 1 +
	...
	for(size_t i = 0; i < pin->buffer->num_samples; i++) {
		...
		if(tts->pretty_print) {
			if(am_siformat_double(doubles[i],
			...
			snprintf(str, DOUBLE_MAX_DECIMAL_DIGITS,
				 "%f", doubles[i]);
		}
