 ...
 * in-memory representation and appends the result to the per-trace array.
 *
...
{{template.getSignature()}}
{
	...
	if({{store_tag.getFunctionName()}}(ctx, &mem)) {
		AM_IOERR_GOTO_NA(ctx, out_err_destroy, AM_IOERR_ALLOC,
				 "Could not store {{tomem_tag.getMemType().getEntity()}} in per-trace array.");
	}
