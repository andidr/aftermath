 ...
 * in-memory representation and appends the result to the subarray identified by
 * dsk->{{gen_tag.getEventCollectionIDDskField().getName()}}, within the
 * appropriate per-event-collection array.
 *
 * Returns 0 on success, otherwise 1.
 */
{{template.getSignature()}}
{
	/* The target in-memory data structure */
...
{# #}
	if({{store_tag.getFunctionName()}}(ctx, dsk->{{gen_tag.getEventCollectionIDDskField().getName()}}, dsk->{{gen_tag.getEventIDDskField().getName()}}, &mem)) {
